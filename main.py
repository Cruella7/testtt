...
class SmofiaWindow(QDialog):

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Smofia")
...
widget = layout.addWidget(self.username_input)
...
class ScheduleWindow(QWidget):
    def __init__(self, set_layout=self.setLayout(layout)):
        super().__init__()
    ...
    def load_schedule(self):
        conn = sqlite3.connect('pairs_users.db')Z
        cursor = conn.cursor()
...
if __name__ == '__main__':
    ...
    Smofia_window = SmofiaWindow()
    if Smofia_window.exec_() == QDialog.Accepted:
        if Smofia_window.status == 'student':
            StudentWindow.show()
        else:
            TeacherWindow.show()
